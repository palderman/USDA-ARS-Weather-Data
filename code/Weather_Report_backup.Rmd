...
```{r wind_direction_day, eval = FALSE}
wind_dir <- wth_data %>% 
  select(wind_dir_day) %>% 
  unique()
```
...
```{r weather_state_tobs, eval = FALSE}
...
```{r weather_state_day, eval = FALSE}
bad_states <- wth_data %>% 
  select(weather_state_day) %>%
  unique() 
...
```{r precip_snowfall_in_inches, eval = FALSE}
wth_data %>% 
  ggplot()+
...
```{r precip_snow_depth_tobs, eval = FALSE}
